                ...
                if (a != window) {
                    ...
                    if (negative > 0) 
                        for (d = 0; d < negative + 1; d++) {
                            ...
                            /* compute dot product between embedding and context token */
                            f = 0;
                            for (c = 0; c < layer1_size; c++) f += syn0[c + l1] * syn1neg[c + l2];

                            /* compute sigmoid of f */
                            if (f > MAX_EXP) g = (label - 1) * alpha;
                            else if (f < -MAX_EXP) g = (label - 0) * alpha;
                            else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
                            ...
                            /* gradient descent on context matrix */
                            for (c = 0; c < layer1_size; c++) syn1neg[c + l2] += g * syn0[c + l1];

                        }

                    /* gradient descent on embeddings matrix */
...
void train() {
    ...
    if (read_vocab_file[0] != 0) restore_vocab(); else build_vocab_from_text();
    if (save_vocab_file[0] != 0) save_vocab();

    if (output_file[0] == 0) return;

    init_matrices();

    if (negative > 0) init_unigram_table();

    start = clock();

    /* launch threads*/
    for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, train_thread, (void *)a);
